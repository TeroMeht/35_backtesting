"""
Thin DuckDB read helpers -- OWN copy, no imports from scanner. Same
schema, same conventions (Europe/Helsinki tz on intraday ts, Python
date on session_date), but a separate module so the backtester
stays independent of scanner's internals.
"""
...
def open_conn(db_path: Optional[str] = None) -> duckdb.DuckDBPyConnection:
    ...
    if not p.exists():
        raise FileNotFoundError(
            f"DuckDB cache not found at {p} -- run 34_dataengine's "
            f"backfiller first.",
        )
...
def read_daily(
    ...
    *,
    ...
    if not df.empty:
        # DuckDB returns DATE as datetime64[us]; normalize to
        # python date so comparisons against session_date scalars
        # elsewhere in the backtester work.
...
def read_intraday(
    ...
    *,
    ...
    falls in [start, end]. Adds `session_date` (local date, python
    ``date`` objects). `ts` is tz-aware Europe/Helsinki.
    """
    syms = list(symbols)
